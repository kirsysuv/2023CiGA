...
public class DialogueEntity
{
    ...
    public class Node
    {
        ...
        public string RoleName 
        {
            get 
            {
                const string ROLELABEL = ":";

                return tags.Where(item => item.Contains(ROLELABEL)).Select(item => item.Substring(item.IndexOf(ROLELABEL) + ROLELABEL.Length)).FirstOrDefault().Replace('-',' ');
            }   
        }
    ...
    public class Dialogue
    {
        ...
        public void ParseTwineText(TextAsset twineText)
        {
            ...
            for (int i = 0; i < nodeData.Length; i++)
            {
                ...
                bool tagsPresent = currLineText.Contains("[") && (currLineText.IndexOf("[") < currLineText.IndexOf("\r\n"));
                int endOftag = currLineText.IndexOf("]");
                ...
                string tags = tagsPresent
                    ? currLineText.Substring(titleEnd + 1, (endOftag - titleEnd) - 1)
                    : "";
                ...
                curNode.paragraphs = messsageText?.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries)?.ToList();
